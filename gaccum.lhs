...
Any function $Time \rightarrow a$ of discrete time (Definition \ref{def:discrete-time}) is equivalent to a countable set of pairs of the type $(Time,a)$, such that for every interval $[t_0,t_1] \subset Time$ there are a finite number of pairs $(t,x): t \in [t_0, t_1],x :: a$. The discreteness of the range does not matter. In this case \emph{scanlT} (the ``temporal scanl'') can be simply \emph{scanl} on a time-ordered sequence of the pairs. Recall the type of \emph{scanl}:
\begin{code}
  scanl :: (a -> b -> b) -> b -> [a] -> [b]
...
Now, for non-discrete temporal values of continuous time, the \emph{scanlT} function can be defined as before except that we use the limit $\Delta t \rightarrow 0$ on the result of applying \emph{scanlT} to the sequence (rather than trying to find the ``stepping points''). The difference between this and the previous cases is that we end up evaluating the temporal value at \emph{all} time points, regardless of whether the value actually changes in their vicinity at all or not. 
\begin{equation*}
